...
try {
  ...
  $processes=Get-CimInstance Win32_Process | Where-Object{$_.ExecutablePath -and $_.Name -ne ""} | Select-Object Name,ProcessId,ExecutablePath | Sort-Object Name

  if(-not $processes){
    Write-Log "No user-level applications found running." 'WARN'
  } else {
    Write-Log "Found $($processes.Count) running applications." 'INFO'
    foreach($proc in $processes){Write-Log "[$($proc.ProcessId)] $($proc.Name) => $($proc.ExecutablePath)" 'DEBUG'}
  }
